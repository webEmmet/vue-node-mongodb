        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressInfoFilter" :class="{'check': checkIndex===index}" @click="checkIndex=index;endAddressId=item.addressId">
                    <dl>
                      <dt>{{item.userName}}</dt>
                    ...
                    <div class="addr-opration addr-del">
                      <a href="javascript:;" class="addr-del-btn" @click="delOperation(item.addressId)">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                    <div class="addr-opration addr-set-default">
                      <a href="javascript:void(0);" class="addr-set-default-btn" v-if="!item.isDefault" @click="setDefaultAddress(item.addressId)"><i>Set default</i></a>
                    </div>
                    <div class="addr-opration addr-default" v-if="item.isDefault">Default address</div>
                  </li>
                  <li class="addr-new" @click="addressInfoModal=true">
              ...
              <div class="shipping-addr-more">
                <a class="addr-more-btn up-down-btn" href="javascript:;" :class="{'open': showLength > 3}" @click="changeShowCount">
                  more
                  <i class="i-up-down">
            ...
            <div class="next-btn-wrap">
               <router-link class="btn btn--m btn--red" :to="{path:'/orderConfirm', query: {addressId: endAddressId}}">Next</router-link> 
            </div>
          </div>
        </div>
      </div>
      <Modal :loginAddCart="addressInfoModal" @closeDialog="addressInfoModal=false">
          <div slot="message">
               <el-form ref="form" :model="addressInfo" label-width="100px">
                <el-form-item label="地址Id:">
                    <el-input v-model="addressInfo.addressId" placeholder="请输入地址Id"></el-input>
                </el-form-item>
                <el-form-item label="收件人姓名:">
                    <el-input v-model="addressInfo.userName" placeholder="请输入收件人姓名"></el-input>
                </el-form-item>
                <el-form-item label="街道名称:">
                    <el-input v-model="addressInfo.streetName" placeholder="请输入街道名称"></el-input>
                </el-form-item>
                <el-form-item label="邮政编码:">
                    <el-input v-model="addressInfo.postCode" type="number" placeholder="请输入邮政编码"></el-input>
                </el-form-item>
                <el-form-item label="电话:">
                    <el-input v-model="addressInfo.tel" type="number" placeholder="请输入电话"></el-input>
                </el-form-item>
              </el-form>          
          </div>
          <div slot="btnGroup">
            <el-button type="success" @click="addAddress">确定</el-button>
            <el-button type="danger" @click="addressInfoModal=false">取消</el-button>
          </div>
      </Modal>
      <Modal :loginAddCart="delAddressModal" @closeDialog="delAddressModal=false">
          <p slot="message">是否删除地址信息</p>
          <div slot="btnGroup">
            <el-button type="danger" @click="conDelAddress">确定</el-button>
            <el-button type="info" @click="delAddressModal=false">取消</el-button>
          </div>
      </Modal>
...
export default{
  ...
  computed: {
    addressInfoFilter() {
      return this.addressInfos.slice(0, this.showLength);
    }
  },
  methods: {
    addAddress() {
      api.post('/users/addAddress', this.addressInfo).then((res) => {
        if (res.data.status === '0') {
          ...
          this.addressInfoModal = false;
          this.getAddressInfo();
        }
      });
    },
    getAddressInfo() {
      api.get('/users/getAddressInfo').then((res) => {
        if (res.data.status === '0') {
          this.totalAddLength = res.data.result.length + 1;
          this.addressInfos = res.data.result;
          res.data.result.forEach((item, index) => {
            if (item.isDefault === true) {
              this.checkIndex = index;
              this.endAddressId = item.addressId;
            }
          });
        }
      });
    ...
    conDelAddress() {
      api.post('/users/delAddress', {addressId: this.addressId}).then((res) => {
        let _res = res.data;
        if (_res.status === '0') {
          this.getAddressInfo();
          this.$notify({
    ...
    delOperation(addressId) {
      this.delAddressModal = true;
      this.addressId = addressId;
    },
    changeShowCount() {
      if (this.showLength === 3) {
        this.showLength = this.totalAddLength;
      } else {
        this.showLength = 3;
      }
    },
    setDefaultAddress(addressId) {
      api.post('/users/setDefaultAddress', {
        addressId: addressId
      }).then((res) => {
        this.getAddressInfo();
      });
