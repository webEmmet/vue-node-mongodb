        ...
        <div class="navbar">
          ...
          <div class="navbar-right-container" style="display: flex;">
            <div class="navbar-menu-container">
              ...
              <a href="javascript:void(0)" class="navbar-link" @click="dialogVisible=true,errTip=false" v-if="!nickName">Login</a>
          ...
          <el-dialog
            ...
            size="tiny"
            >
            <el-tabs type="card" v-model="tabsName">
              <el-tab-pane label="登录" name="login">
                <el-input class="input-text" v-model="userName" placeholder="请输入用户名"></el-input>
                <el-input class="input-text" v-model="userPwd" placeholder="密码" @keyup.enter.native="submitInfo"></el-input>
                <!-- <input type="text" v-model="userPwd" placeholder="密码"> -->
                <span class="errTip" v-if="errTip">您输入的账号或密码错误</span>
                
              </el-tab-pane>
              <el-tab-pane label="注册" name="register">
                <el-input class="input-text" v-model="userName" placeholder="请输入用户名"></el-input>
                <el-input class="input-text" v-model="userPwd" placeholder="请输入密码"></el-input>
                <el-input class="input-text" v-model="userPwdRepeat" placeholder="请重复输入密码" @keyup.enter.native="submitInfo"></el-input>
                <span class="errTip" v-if="unusedName">您输入的用户名已被使用</span>
                <span class="errTip" v-if="pwdRepeateErr">两次输入的密码不同</span>
              </el-tab-pane>
...
export default{
  ...
  methods: {
    submitInfo() {
      ...
      let tab = this.tabsName;
      if (tab === 'register') {
        if (this.userPwd !== this.userPwdRepeat) {
          this.pwdRepeateErr = true;
          return;
        }
      }
      api.post('/users/' + tab, {
        ...
        if (_res.status === '0') {
          ...
          // this.userName = _res.result.userName;
          this.$store.commit('updateUserInfo', _res.result.userName);
          this.errTip = false;
          this.unusedName = false;
          this.getCartCount();
        } else {
          tab === 'login' ? this.errTip = true : this.unusedName = true;
          // this.errTip = true;
        }
      });
    },
    ...
    checkLogin() {
      api.get('/users/checkLogin').then((res) => {
    ...
    getCartCount() {
      api.get('/users/getCartCount').then((res) => {
        res = res.data;
        if (res.status === '0') {
